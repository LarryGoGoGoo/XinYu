...
class BaseModel(models.Model):
    ...
    def __CreateByReq(self, model, params):
        ...
        # 所有业务表 id 均为 bigint auto_increment 自增主键，交给数据库生成，
        # 不再用 uuid 覆盖（uuid4().int >> 64 可能溢出有符号 BIGINT，导致随机插入失败）
        ...
        for k, v in params.items():
            if k in column_list:
                paramss[k] = v
        # 仅当模型确实有 addtime 字段时才注入（config 等表无 addtime，避免 TypeError）
        if "addtime" in column_list:

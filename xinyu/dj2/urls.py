...
urlpatterns = [
    ...
    path('xinyu/',include('main.urls')),#兼容前端硬编码的旧API前缀
...
if os.path.isfile(os.path.join(os.getcwd(),"templates/front/index.html")):
    ...
    # 旧用户端入口(front/index.html)已删除,根路径与旧入口统一 302 跳转到用户端 H5,
    # 避免评委/验收人员点根路径时 500 白屏
    H5_HOME = '/xinyu/front/h5/index.html'
    urlpatterns.extend([
        path(r'', RedirectView.as_view(url=H5_HOME, permanent=False)),
        path(r'index.html', RedirectView.as_view(url=H5_HOME, permanent=False)),
    ])
